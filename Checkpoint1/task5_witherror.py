...
def kawasaki_ising_sim(nstep, N, kT):
    ...
    return meanEnergy, energyError, heatCapacity, heatCapacityError
...
f = open('task5_with_error_results', 'w')
for temp in temps:
    energy, eerror, heatCap, hCerror = kawasaki_ising_sim(11000, 50, temp)
    f.write(f'{temp} {energy} {eerror} {heatCap} {hCerror}\n')

    energyAvgs.append(energy)
    energyErrors.append(eerror)
    heatCaps.append(heatCap)
...
axes[0].errorbar(temps, energyAvgs, yerr=energyErrors, fmt='o')

axes[1].scatter(temps, heatCaps, marker='o', color='RoyalBlue')
axes[1].set_xlabel('Value of kT')
axes[1].set_ylabel('Heat capacity per spin')
axes[1].errorbar(temps, heatCaps, yerr=hCerrors, fmt='o')

plt.savefig('Task5_with_errors_fig')
plt.close()