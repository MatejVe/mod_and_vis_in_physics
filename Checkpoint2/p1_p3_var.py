...
#f = open('p1_p3_var.dat', 'w')
#for i, p1 in enumerate(p1s):
#    row = []
#    for j, p3 in enumerate(p3s):
#        print(f'p1-{i}/20, p3-{j}/20')
#        s = SIRS(50, p1, 0.5, p3)
#        var = s.infectiveness_variance_measure(1100, 100)
#        f.write(f'{p1} {p3} {var}\n')
#        row.append(var)
#    plotData.append(row)
#f.close()

f = open('p1_p3_var.dat')
data = f.readlines()
...
plt.colorbar()
plt.title('Variance of the number of infected sites\n vs different spreading parameters, $p_2=0.5$')
plt.xlabel('$p_1$')
plt.ylabel('$p_3$')
plt.savefig('p1_p3_var')
plt.close()